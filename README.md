...
바텐딩 액체는 공통 `ILiquidSimulationBackend` 계약 아래 두 구현을 선택할 수 있습니다.

| 모드 | 구현 | 용도 |
|---|---|---|
| `LegacyRigidbody2D` | `LiquidPool` 기반 Unity Physics2D 입자 | 기존 풀링·재활용 최적화 경로를 명시적으로 사용 |
...
- 레거시 경로의 입자 풀 캐시, 화면 경계 계산, 자동 반환, 중복 혼합 방지 및 수면 처리는 기존 `LiquidPool`/`LiquidReaction` 최적화 구현을 유지합니다.
- GPU PBF/XPBD는 `Infrastructure/GpuFluid`와 `Runtime/Liquid/Gpu`에 분리된 신규 백엔드입니다.
- `BottleController`, `VesselLiquidTracker`, 젓기·셰이킹 처리는 두 백엔드를 연결하는 공용 통합 계층입니다.
...
- 성능 계측: `LiquidStressHarness`의 입자 수와 p95/p99 프레임 시간을 사용
