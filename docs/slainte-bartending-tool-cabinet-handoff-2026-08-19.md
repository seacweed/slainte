...
- 기준일: 2026-08-19
- 프로젝트: `C:\unity_proj\slainte\slainte`
...
- 도구장 배경:
  ...
  `C:\Users\boguk\Downloads\아트\tool_cabinet.png`

- 도구:

  `C:\Users\boguk\Downloads\아트\도구\`

- 얼음:

  `C:\Users\boguk\Downloads\아트\얼음\`
...
도구장 배경, 도구 17개, 얼음 3개는 아직 `Assets`에 반입되지 않았다.

병 43개와 임시 잔 4개만 현재 `Assets/Art/Bartending`에 반입되어 있다.
...
아래 네 항목은 권장안이며 아직 최종 확정 사항은 아니다.

- 도구별 실물 인스턴스는 한 번에 하나
- 내용물이 든 용기는 도구장으로 반환 불가
- 서빙 잔은 동시에 하나만 사용
- 주문 종료 시 도구 자동 정리

구현 전에 필요하면 사용자에게 확인한다.
...
## 16. 구현 전 추가 확인이 필요한 UX

아래 항목은 아직 최종 확정되지 않았다.

- 도구별 실물 인스턴스를 한 개만 허용할지
- 내용물이 든 지거·셰이커·잔을 도구장에 반환할 수 있는지
- 반환 시 내용물을 버릴지
- 서빙 잔을 동시에 한 개만 꺼낼지
- 주문 종료 시 도구를 자동 반환할지
- 사용자가 직접 정리해야 하는지
...
사용자 확인 없이 권장안을 확정 기획으로 기록하지 않는다.
...
- 도구장 배경·도구·얼음 아트는 아직 Assets에 반입되지 않았다.
- 도구장은 아직 기능 구현 전이다.
- 다음 구현은 기존 `DrawerArea`를 기반으로 시작한다.
- 이 문서를 만든 작업에서는 Unity 코드, 씬, 에셋을 변경하지 않았다.
