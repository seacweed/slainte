...
- `RecipeSearchUI.Awake()`에서 `ItemDefCatalog.LoadFromResources("Items")` + `CocktailRecipeDataLoader.LoadDefault(itemCatalog)`로 카탈로그를 로드(`BusinessOrderSessionController`와 동일한 패턴)한 뒤 `appearsInRecipeBook == true`인 레시피만 캐시. 변형(`Variants/` 하위 87개)은 전부 `appearsInRecipeBook = false`라 자동 제외됨.
