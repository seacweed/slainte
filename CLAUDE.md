...
- **런타임 상태 Source of Truth**: `GameProgress` (flags, completedEpisodeIds, affinityVars, boardSlots, currentDay, currentChapterId, currentMoney, reputation, 당일 정산 집계, 손님 등장 횟수)
...
- **제조 판정 분기**: `requiresCrafting` 노드는 `CraftingJobResult`(Good/MidIce/MidGlass/MidIceGlass/MidWrongMenu/Bad 6종) 결과별로 `EpisodeNode.craftingOutcomes`(`List<CraftingOutcome>`)에서 다음 노드·플래그·변수 변경을 분기. 실제 재료/얼음/잔 비교로 결과를 자동 판정하는 로직은 아직 없어 `CraftingJudgeUI`의 버튼 6개로 수동 판정. 레시피 기반 자동 판정(`BusinessOrderSessionController`의 `CocktailEvaluator` 연동)으로 대체하려던 시도가 있었으나 현재는 되돌려져 수동 판정으로 유지 중 — 재통합 예정이며, 그래프 asset에는 이미 `CraftingRecipeId` 값이 남아있지만 `EpisodeNode`/`EpisodeEventData`엔 대응 필드가 없어 지금은 저장할 때마다 버려짐
- **영업 주문 세션**: `Assets/Scripts/Business/`의 `BusinessFlowBootstrap`(BusinessScene 진입 시 자동 설치) → `BusinessOrderSessionController`(주문 제시·제조 진입·판정·보상·완료 상태머신)가 영업(`StartBusinessSequence()`)과 에피소드 제조 노드(`StartEpisodeOrder()`) 양쪽의 주문 처리를 공유. 단, 하루치 손님 순번을 실제로 진행시키던 `BusinessSequenceRunner`/`BusinessSequencePlanner`의 공개 생성 API는 현재 제거된 상태 — `StartBusinessSequence()`가 모드 전환만 하고 손님 큐를 진행하지 않음(재구현 보류 중)
- **네임스페이스**: 바텐딩·영업 신규 코드(`Assets/Scripts/Bartending/`, `Assets/Scripts/Business/`)는 `Slainte.Bartending`/`Slainte.Business` 네임스페이스를 쓰고, 그 외 기존 코드(`GameProgress`, `EpisodeManager`, `EpisodeRunner` 등)는 네임스페이스 없이 global로 작성됨 — 두 컨벤션이 공존하므로 새 파일은 해당 폴더의 기존 관례를 따를 것
